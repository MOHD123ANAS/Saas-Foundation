...
def serialize_subscription_data(subscription_response):
    ...
    subscription_items = subscription_response["items"]["data"] if "items" in subscription_response else []
    ...
    if subscription_items:
        current_period_start_ts = subscription_items[0].get("current_period_start")
        current_period_end_ts = subscription_items[0].get("current_period_end")

    current_period_start = date_utils.timestamp_as_datetime(
        current_period_start_ts) if current_period_start_ts else None
    current_period_end = date_utils.timestamp_as_datetime(current_period_end_ts) if current_period_end_ts else None
    return {
...
def cancel_subscription(stripe_id,reason="",feedback="other",cancel_at_period_end=False,raw=True):
    if cancel_at_period_end:
        response = stripe.Subscription.modify(stripe_id,cancel_at_period_end=cancel_at_period_end,
